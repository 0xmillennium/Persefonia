...
public final class InMemoryContentRevisionRepository implements ContentRevisionRepository {
    ...
    public Optional<RevisionNumber> findLatestRevisionNumber(ContentId contentId) {
        return findByContentId(contentId).stream().map(ContentRevision::revisionNumber).max(Comparator.comparingInt(RevisionNumber::value));
    }

    public List<ContentRevision> all() {
