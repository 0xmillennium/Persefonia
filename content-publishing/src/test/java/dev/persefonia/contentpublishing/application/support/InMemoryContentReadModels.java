...
public final class InMemoryContentReadModels implements
    ...
    public InMemoryContentReadModels(
    ...
    public List<TranslationCandidateItem> candidatesFor(TranslationGroupId groupId, ContentType contentType) {
        var group = translationGroups.findById(groupId).orElseThrow();
        return manageableContent()
    ...
    private PublicTaggedContentItem taggedItem(ContentItem item) {
    ...
    private String publicUrl(ContentItem item) {
        Slug slug = item.slug().orElseThrow();
    ...
    private record RenderableContent(
            ContentId contentId,
