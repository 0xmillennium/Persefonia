...
final class PublicContentReadModelMapper {
    ...
    static Optional<PublicContentPageResult> toPageResult(ContentItem item) {
        return item.metadata().canonicalPath().flatMap(canonicalPath -> toPageResult(item, canonicalPath));
    }

    static Optional<PublicContentPageResult> toPageResult(ContentItem item, CanonicalPath canonicalPath) {
