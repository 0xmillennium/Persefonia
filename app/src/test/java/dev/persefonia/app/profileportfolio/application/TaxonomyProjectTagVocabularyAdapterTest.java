...
class TaxonomyProjectTagVocabularyAdapterTest {
    private final TagVocabularyQueryService queries = mock(TagVocabularyQueryService.class);
    private final TaxonomyProjectTagVocabularyAdapter adapter = new TaxonomyProjectTagVocabularyAdapter(queries);
    ...
    void mapsAssignableAndAssignedTagReadModelsWithoutLeakingTaxonomyTypes() {
        ...
        assertThat(adapter.findAssignableTags()).extracting(option -> option.id()).containsExactly(active.id().value());
    ...
    void validationRejectsMissingAndNewArchivedButAllowsExistingArchived() {
        ...
        when(queries.findByIds(Set.of(archived.id(), dev.persefonia.taxonomy.domain.model.TagId.from(missing.value()))))
        ...
        var accepted = adapter.validateAssignments(Set.of(archivedReference), Set.of(archivedReference));
        assertThat(accepted.missingTagIds()).isEmpty();
        assertThat(accepted.newlyArchivedTagIds()).isEmpty();
    }

    private static TagVocabularyItem item(TagStatus status) {
        return new TagVocabularyItem(dev.persefonia.taxonomy.domain.model.TagId.newId(), status.name(), status.name().toLowerCase(), status);
    }
