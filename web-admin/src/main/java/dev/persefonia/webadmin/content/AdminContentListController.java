...
public final class AdminContentListController {
    ...
    public String list(Authentication authentication, CsrfToken csrfToken, Model model) {
        var actor = actors.resolve(authentication);
        model.addAttribute("page", views.list(chrome.create(authentication, csrfToken), queries.listEditableContent(actor)));
